...
public class HibernateQueryUserByNameAndPasswordTest extends TestCase {
	...
	public static void testOneToOneAndCommitTx() {
		...
		NamedQueryChessBoardPreference preference = new NamedQueryChessBoardPreference();
		preference.setChesswidth(80);

		NamedQueryUserName userName = new NamedQueryUserName();
	   ...
	   public static void testQueryUser() {
	    	Session session=factory.openSession();
	         Query query=session.createQuery("from NamedQueryUserName where userName=:userName and password=:password");
	    query.setParameter("userName", "olivia");
	        query.setParameter("password", "olivia");
	        NamedQueryUserName user=(NamedQueryUserName)((org.hibernate.query.Query) query).uniqueResult();
	    if(user!=null) {
	   ...
	   public static void testQueryUserByNamedQuery() {
	    	Session session=factory.openSession();
	         Query query = session.createNamedQuery("checkCredentials", 
	        		 NamedQueryUserName.class);
	        		query.setParameter("userName", "olivia");
	        		query.setParameter("password", "olivia");
	        		NamedQueryUserName user = (NamedQueryUserName) query.getSingleResult();
	    if(user!=null) {
	       System.out.println("username and password are valid");
	    }else {
	       System.out.println("username and password are not valid");
